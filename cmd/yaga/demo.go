// demo.go
//
// Implements `yaga init --demo`: scaffolds a full-featured sqlite demo
// project built around an order-management domain. Besides the yaga.yaml
...
// yaga.yaml (full-featured order-management config), sql/migrations/schema.sql
...
// is given.
// Params: configPath (path of the yaga.yaml file to write), outDir (output
...
func cmdInitDemo(configPath, outDir, adminPassword string, force bool) error {
	...
	fmt.Println("  1. yaga generate --config", configPath, "--out", outDir)
...
// demoYAML returns the yaga.yaml config for the demo project. It exercises
