// ai.go
//
// D7 — AI-assisted config editing (yaga edit via OpenRouter or a local LM
// Studio server). Non-interactive and opt-in: the AI path only runs when
// `yaga edit --prompt "…"` is given, otherwise the TUI editor runs
...
const (
	...
	// full multi-KB yaga.yaml; the old 90s deadline regularly fired on them.
...
func buildEditPrompt(cheatsheet, currentYAML, instruction string) []chatMessage {
	return []chatMessage{
		{
			Role: "system",
			Content: "You edit a yaga.yaml admin panel configuration. Return ONLY the changed sections of the config " +
		...
		{
			Role:    "user",
			Content: "yaga.yaml schema cheat-sheet:\n\n" + cheatsheet + "\n\nCurrent yaga.yaml:\n\n```yaml\n" + currentYAML + "\n```\n\nInstruction:\n" + instruction,
		},
	}
}
...
// extractYAMLBlock pulls the proposed yaga.yaml out of a model reply. It
...
// topLevelKeys is the set of keys the root of yaga.yaml may contain. A
...
// mergeYAML merges a model fragment into the current yaga.yaml. Both are
