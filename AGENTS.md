...
**Sync screen** (`sync.go`): file-level SQL↔YAML analysis via `internal/schema` — `ParseSchema` (globs `sql/migrations/*.sql`; takes explicit paths), `ParseQueries(sql/queries)`, `CollectReferences(cfg)` (query/table/column refs from YAML incl. options_query + nested page widgets). Renders a colored report of missing queries/tables/columns and FK-target `List{Table}` queries, with apply buttons: "Generate missing queries" (writes `{table}.sql` per schema table, **never overwrites** existing files) and "Import resources from schema" (`GenerateResourceYAML` → appended to `cfg.Resources`, marks modified). Driver comes from `schema.Driver(cfg)`.

**Preview screen** (`preview.go`): ASCII-frame mock of the dashboard (topbar + sidebar from `cfg.Navigation` + per-page widget boxes) and per-resource list mock. No DB, no generated app.
