...
| E2 — Lua scripting for actions & hooks | gopher-lua runtime in the generated app; `script:` bodies with `ctx` scope + `db.exec/query/query_one`/`abort`/`log` host API, `?`→`$N` driver-aware renumbering, 5 s timeout, audit-tx integration; script bodies editable in TUI (TextArea), wedit (Lua-highlighted) and via agent/MCP. | **Implemented** (2026-08-16) |
